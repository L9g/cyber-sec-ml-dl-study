...
def _(mo):
    ...
    > **本系列：01 故意踩坑 → 03（本文）逐一封堵 → 04 跨数据集泛化测试**
...
def _(mo):
    ...
    > 取决于模型在**真实部署分布**上的 FPR。一旦换到模型没见过的数据集（notebook 04），
    > FPR 飙升，base-rate 才显出它真正的破坏力。这张表的方法本身才是卖点。
    """)
    return
...
def _(ev, m_bal, pd):
    ...
    print( "  投射到真实占比——本数据集 FPR≈0 故仍可用；换到 LODO（notebook 04）FPR 飙升时，")
    print( "  同一张表会显示告警台被误报淹没。这正是「在哪测」决定「能不能用」。")
    return
