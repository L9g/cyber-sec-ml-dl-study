...
def _(mo):
    ...
    **稳健头条 = 量级落差**：分布内 PR-AUC/macro-F1 ≈ 1.0（§1，合成数据平凡可分）；6 对跨数据集
    ...
    > （攻击流概率被自信压到 0.4 以下，微降阈值救不回、低阈值要付 FPR 代价）；「N/6 低于随机」这个二元计数也随训练量翻转。
...
def _(mo):
    mo.md(r"""
    **没有任何跨集对的 PR-AUC 接近分布内 ~1.0（实测 max≈0.72，UNSW→ToN），6 对均值≈0.25，崩向随机。**
    ...
    说明不是单纯「阈值太保守」、把阈值从 0.5 微降到 0.4 救不回（更低阈值或能换回部分 recall，但代价是 FPR/告警预算，
    需以 recall@fixed-FPR 评估），根因是分布漂移下评分的整体重组。
    ...
    - notebook 04：换一个数据集，PR-AUC 崩向随机基线、macro-F1 跌到 ≈0.4；且（§2.2 P6）**四个模型族都无法恢复分布内表现**。
    ...
    3. **加模型容量救不回**：换 LogReg/MLP/单特征 stump 都无法恢复分布内表现（§2.2），失败根因是评估/数据不是模型选择。
...
def _(lodo_df):
    ...
    def test_pr_auc_magnitude_collapses():
        # 稳健头条（量级，非计数）：没有任何跨集对的 PR-AUC 接近分布内 ~1.0（实测 max≈0.72，UNSW→ToN）；
        # 且 6 对均值远低于 1.0（实测≈0.25）。两条都对种子/版本鲁棒。
