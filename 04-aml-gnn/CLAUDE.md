...
- **【2026-07-01 原生 actor 模型已做（Reference 起步），见 `notebooks/06` + `reports/native-actor-vs-projection.md`】** 51 维地址特征直接建模（不碰交易分数）、group-aware **inductive** split（test=首现>34、无实体泄漏，`src/data.py::native_actor_temporal_split`+单测）。核心诚实发现:**原生独立信号很弱（PR-AUC 0.297）而 tx 投影碾压（0.741）**;5% who-catches 不对称（仅 proj 1,468 ≫ 仅 native 128）→ 原生是投影的更弱子集。**gap = guilt-by-association 循环显形**（投影下游于「是否触过 illicit 交易」这个定义标签的信号、几乎按构造匹配标签；0.30 才是地址特征诚实上限）。→ 旗舰问句「actor 有无独立信号」= 有但很少、大头是循环标签回收，加固 provenance；「切分干净（inductive）≠ 标签干净（全局/事后）」。给 GNN 立更硬靶（须同打过 0.30 与 0.74、且证增益来自结构/时序）。绝对时间 block 列（first/last_block_appeared_in、first_sent/received_block）= 泄漏、已剔（`WALLET_ABSOLUTE_BLOCK_FEATURES`）。
- **【2026-07-01 交易图 GNN 已做（Reference），见 `notebooks/07` + `reports/tx-graph-gnn-does-graph-help.md`；CPU 无 GPU，torch/PyG 已进 requirements】** 诚实**三层对照**（隔离两个混淆）：LightGBM(GBDT 无图) **0.813** / MLP(NN 无图) **0.624** / GraphSAGE(NN+消息传递) **0.660**。**纯消息传递增益 SAGE−MLP = +0.036**（图确有微弱同期信号、非零），**大头缺口是 NN vs 树 −0.19**——⚠️ **只报「GNN<GBDT」会把账错记到图头上**，必须加 MLP 控制项。机制：交易图**按时间步断开**（nb07 复验所有边 Δt=0 → 消息传递只在同期、temporal split 图层面天然 inductive）→ 同期一跳聚合边际小。实证「图 > 表格不自动成立」（Weber 2019）；图的**大**增益只可能来自跨期时序 → 交易图跨期断开，要到**地址图**（AddrAddr 2.87M 边有跨期边）才谈得上，按无-GPU 记忆（[[project-compute-no-gpu-cloud-fallback]]）留云主机/邻居采样。config.seed_everything 已扩展 torch 种子。
