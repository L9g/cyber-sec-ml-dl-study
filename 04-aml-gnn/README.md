...
9. ✅ **地址图 GNN（`notebooks/08`+`09`，Reference，CPU 无需 GPU）——「到了有跨期边的连通图，图就赢了吗」**：nb08 先证实前提——地址图是**单块巨型连通分量（99.999%）**、**~19.7% 边跨期**（交易图 0%），但**仅 ~2.88% 边跨 ≤34/>34 的 split 边界**（inductive 通道很窄）、unknown 占 67.8%。nb09 同款三层对照（strict inductive，n_test 92,451）：GBDT **0.246** < MLP **0.346** < GraphSAGE **0.361**。**纯消息传递增益 SAGE−MLP = +0.015**（图确有微弱正信号），且**与交易图赢家反转**（tx 树赢/addr NN 赢，两处消息传递均只微弱正增益）。**主线不变**：即便上有跨期边的连通图，SAGE 0.361 仍远低于 tx 投影 0.74——图修不了标签口径（guilt-by-association 回溯循环仍在）。分步说明（含建图方法/LightGBM 参数/跨期边/对照设计文献）见 [`reports/address-graph-gnn-does-graph-help.md`](reports/address-graph-gnn-does-graph-help.md)
