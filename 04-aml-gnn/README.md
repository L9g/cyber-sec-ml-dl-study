...
9. ✅ **地址图 GNN（`notebooks/08`+`09`，Reference，CPU 无需 GPU）——「到了有跨期边的连通图，图就赢了吗」**：nb08 先证实前提——地址图是**单块巨型连通分量（99.999%）**、**~19.7% 边跨期**（交易图 0%），但**仅 ~2.88% 边跨 ≤34/>34 的 split 边界**（inductive 通道很窄）、unknown 占 67.8%。nb09 同款三层对照（strict inductive，n_test 92,451）：**未加权 GBDT 0.315 / 加权 0.246** < MLP **0.346** < GraphSAGE **0.361**。**纯消息传递增益 SAGE−MLP = +0.015**（图确有微弱正信号）；**真实 NN vs 树反转仅 +0.031**（须用同群体未加权 GBDT 作基准）——⭐**方法学盒子**：排序度量（PR-AUC）下**类别加权非免费**，加权把树从 0.315 压到 0.246（净亏 0.069），拿加权当基准会把反转虚报成 +0.100（三倍）；**反转经调参守门**（未加权 GBDT 12 组网格最优 0.322 仍 < MLP 0.346 → 非欠调假象）；**与交易图赢家反转**（tx 树赢/addr NN 微赢，两处消息传递均只微弱正增益）。**主线不变**：即便上有跨期边的连通图，SAGE 0.361 仍远低于 tx 投影 0.74——图修不了标签口径（guilt-by-association 回溯循环仍在）。分步说明（含建图方法/LightGBM 参数/跨期边/对照设计文献）见 [`reports/address-graph-gnn-does-graph-help.md`](reports/address-graph-gnn-does-graph-help.md)
10. ✅ **表格基础模型 TabPFN（`notebooks/10`，Reference，CPU 无需 GPU）——「换更强的表格模型，修得了标签口径吗」**：六层对照（strict inductive，共享 8000 分层 test 子样本，base 5.29%）——LightGBM(全train) **0.330** / MLP **0.366** / GraphSAGE **0.374** / **LightGBM@2k 0.383** / **TabPFN(开放版 v2, in-context 2k) 0.308** / **TabPFN+G2T-FM 0.302**。三条关键：(a) **基础模型 vs 等数据树 = TabPFN−LightGBM@2k = −0.074**（⚠️受限配置：n_est=1/2k context/开放版，主线不靠它，uncapped 留云）；(b) **⭐ G2T-FM 结构列增益 = −0.006 ≈ 0**——预注册探针「邻居 illicit 比例」(=guilt-by-assoc 结构版，只用训练标签防泄漏)**加不了分**，与 nb09 消息传递 +0.015 同因（2.88% 跨界窄通道，结构标签信号传不到 test 节点）→ **更证瓶颈是标签口径不是特征/结构表达力**；(c) **距 tx 投影天花板 +0.43**（TabPFN 家族最好 0.308 << 0.74）。**主线加固一档**：从「TabPFN 一家」到「2024–2026 tabular DL SOTA 谱系补不上标签口径的坑」。分步说明见 [`reports/tabpfn-foundation-model.md`](reports/tabpfn-foundation-model.md)
11. Reference 续（需 GPU/云主机，见 [`../docs/cloud-gpu-experiment-plan.md`](../docs/cloud-gpu-experiment-plan.md)）：TabPFN-2.5/3（大 context+全测试集）· **TabDPT**（真实表预训练 FM+retrieval）· EvolveGCN（49 快照时序用满）· G2T-FM 全结构列。本机先做（加固表格上限，不必等云）：RealMLP+meta-GBDT defaults、TabM
