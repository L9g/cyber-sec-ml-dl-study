...
6. 扇出其余聚合（mean/sum/top-k——它们才让 transaction-first 与 actor-first 队列真分叉、投影损失非零；max 下两者等价 Jaccard=1）
7. 之后才做原生 actor-level 模型
8. 最后 Reference 档加 GNN
...
| 档 | 内容 | 状态 |
|---|---|---|
| **MVP** | EDA ✅ + temporal LightGBM baseline ✅ + 非 GNN 对照 ✅ + actor 投影/agreement ✅ + 标签来源审计 ✅ + Setting C ✅ + 归因表雏形 ✅ | ✅ **薄切片跑通**；下一步：扇出 mean/sum/top-k 聚合 或 改 docs/*-v2 项目四章节 |
