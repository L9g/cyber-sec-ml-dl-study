...
- 架构决策记录：`docs/adr/`（0001 到 0016 的完整决策弧）
...
在此之上又落了差异化层「标准到保证结论」的上半边：一个把裁定溯源升成结构的 typed `verdict_provenance`，加一个纯函数 Claim 层，产出多维的 `confidence_basis`（warrant）而不是一个会把覆盖率、证据质量、目标保真度压成一个数字的单一置信度档。这一步据真实摩擦而定，做法与纪律记在 ADR-0016。这条 Claim 层也已用真实模型跑通验证：一次多目标真跑里，系统在无正对照时正确地拒绝把裸跑零成功率当成「安全」，在两臂溯源指纹漂移时正确地拒绝断言防御 delta，都 fail-closed 而非编造结论。
