...
**A note on testing.** The project has two layers of tests, deliberately kept apart. Deterministic code contracts use pytest unit tests, pinned to exact values and boundaries. Narrative regressions, which depend on model scores, use only directional assertions with a margin, and hard thresholds are forbidden, because scores drift with seed, version, and image. There are 128 test functions in all, and the deterministic-slice portion runs entirely offline, with no external dependency.
...
One case shows the force of this discipline better than any other, and it concerns a field that has, to this day, not been added: `verdict_source`.

The background runs as follows. Section 3 noted three sources of a verdict: a deterministic rule, an AI run, and a human review. By the fourth slice a pattern was plain. Each shape of adjudication stashes its verdict source in a different place. Only the AI slice has a typed home for it, namely `ai_run_record`; the other three, a rule-version string, an execution receipt, and a review record, are all tipped into the free-form `measurement_context` dictionary. A unified `verdict_source` field all but demands to be written, and the signal has now been confirmed, four slices over, four times.

The habit of many projects would be to abstract the moment a signal is that strong. Here it was deliberately not added. The reason: to this day, not one downstream consumer has broken for want of the field. The CoverageLedger reads only `status`, `verdict_mode`, and `severity`, and all four shapes run through it. The discipline of letting friction decide the schema demands exactly this restraint: however strong the signal, so long as no real consumer has been driven to the point of needing to tell verdict sources apart, the field is not added. The trigger that will finally force it is well defined. When a consumer appears that must distinguish verdict sources programmatically and assign them different confidence, a future Claim or Assurance engine, say, the fixtures of the four existing slices can be used to reverse-engineer exactly the shape the field should take. Added then, it will be far more accurate than a guess made now from one or two shapes.

Writing "we saw a strong signal and still held fire" into a design document is itself meant to show the reader that this method is not a slogan. The project's fifteen architecture decision records, 0001 through 0015, form one continuous arc of decisions, recording at each step what friction was in play and what field was, or was not, added. They are the traceable evidence for the discipline.

---

## 8. Status, and how to run it

**Where it stands.** All four slices are merged into the mainline and the tests are green. The built layer has demonstrated its cross-domain generality across four shapes of adjudication and four controls, and the cross-control CoverageLedger has begun. It bears saying clearly that the project is still at the slice-validation stage. What it has validated is that the shape and the discipline of this conclusion layer hold up, not that it is a finished, polished platform. Fanning out across more controls, scenarios, and attack families, together with modules such as an experiment manager and the Claim and Assurance engines, remains on the road ahead.
...
- `docs/adr/`: the complete arc of decisions, 0001 through 0015.
