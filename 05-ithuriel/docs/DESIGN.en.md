...
**Security and utility must be adjudicated as a pair.** See `joint_verdict` in section 3. Report the security axis alone and a defence that "aborts the task the moment it detects an injection" looks perfect, when in fact it has traded away all usefulness and completed not a single task. The question this raises, whether aborting the task counts as safe at all, opens onto a whole security-philosophy stance; the appendix at the end of this document sets it out, and explains how it decides the way the report presents things.
...
The "built" half is roughly two thousand six hundred lines of Python across thirteen modules, ordered here most-central first:
...
- `report.py`: the internal trial report view, projecting Claims and the CoverageLedger into a two-dimensional matrix of coverage against warrant and fidelity, and a report that answers the three questions (pure-function rendering, no re-adjudication).
...
**A note on testing.** The project has two layers of tests, deliberately kept apart. Deterministic code contracts use pytest unit tests, pinned to exact values and boundaries. Narrative regressions, which depend on model scores, use only directional assertions with a margin, and hard thresholds are forbidden, because scores drift with seed, version, and image. There are 160 test functions in all, and the deterministic-slice portion runs entirely offline, with no external dependency.
...
Writing both halves into a design document, first "we saw a strong signal and still held fire," then "we moved only once a real consumer forced it, and even then we held the shape and the restraint," is meant to show the reader that this method is not a slogan. The project's seventeen architecture decision records, 0001 through 0017, form one continuous arc of decisions, recording at each step what friction was in play and what field was, or was not, added. They are the traceable evidence for the discipline.

---

## 8. Status, and how to run it

**Where it stands.** All four slices are merged into the mainline and the tests are green, 160 test functions in all. The built layer has demonstrated its cross-domain generality across four shapes of adjudication and four controls, and the cross-control CoverageLedger has begun. On top of that, the upper half, from standards to conclusion, has landed its thinnest layer: a typed `verdict_provenance` and a pure-function Claim layer producing a multi-dimensional warrant (sections 3 and 7). That layer has also been exercised against real models once: across a multi-target run, the system refused to read a bare zero success rate as "safe" where no positive control existed, and refused to assert a defence delta where the two arms' provenance fingerprints had drifted, failing closed in both cases rather than fabricating a conclusion. On top of that, this pairing of Claim and CoverageLedger has now been read by its first real consumer, the internal report view aimed at operators and assessors, which projects the two into a two-dimensional matrix of coverage against warrant and fidelity and a bounded, three-question report, and which deliberately produces no single overall safety score (see section 6 and the appendix). It bears saying clearly that the project is still at the slice-validation stage. What it has validated is that the shape and the discipline of this conclusion layer hold up, not that it is a finished, polished platform. What has landed is the thinnest pure-function Claim layer; a full Claim and Assurance engine, which must handle applicability, claim subject, time validity, and assessor independence rather than merely rolling up several Findings, together with fanning out across more controls, scenarios, and attack families, remains on the road ahead.
...
## Appendix: when a detector aborts the task, is that safe?

This section takes one recurring question and sets it out on its own, because it reaches the foundation of the project rather than any detail of implementation. The scenario is concrete. Point an AI agent at a prompt-injection attack, add an injection detector, and the detector, the moment it smells an injection, aborts the whole task. The attacker has not hijacked the agent into doing anything harmful, but the agent no longer does the legitimate job it was there to do. So: is the system safe?

The honest answer is that the question is malformed. "Safe" is not a property that a single yes or no can carry. Aborting the task is safe along one axis and, precisely, unsafe along another, and the urge to flatten it into one overall verdict is exactly the move this project exists to refuse. Take it a step at a time.

**"Safe" is really several distinct properties bundled together.** At least three want separating. The first is integrity: whether the attacker got the agent to do something harmful, such as exfiltrating data, sending an unauthorised email, or executing an injected command. On this axis, aborting is a success, because the agent refused the injected instruction and did nothing bad. The second is availability: whether the agent, under attack, still does its real work. On this axis, aborting is a failure, because a single injected string put it out of action. The third is harmlessness: whether the system caused injury, and since aborting takes no action at all, it is clean on this one. So "is it safe" depends on which of these you are asking. It withstood the attack on integrity and harmlessness, and it collapsed on availability, and no single word captures both.

**Do not overlook that the attacker still won something.** Even without a hijack, the attacker turned a piece of untrusted content that should have been mere data, a calendar-event description or the body of an email, into control over the agent's availability. That is a genuine, security-relevant capability: an injection-triggered denial of service. So this is not "no problem, perfectly safe." The system has a real weakness, namely that untrusted content can halt it. Whether the weakness matters is another question, but it is a weakness, and there is no getting around that.

**This is, in reliability terms, an old problem.** In the language of RAMS it is the distinction between fail-safe and fail-operational. Fail-safe means that on a fault the system moves to a safe state, usually a stop, as a train brakes to a halt when signalling fails: safe, but unavailable. Fail-operational means that on a fault the system must keep working, if only in a degraded mode, as an aircraft's flight controls must, because stopping is itself lethal. Aborting on injection is fail-safe, not fail-operational. And whether fail-safe is safe enough turns on whether the loss of function is itself a hazard. It is the same class of judgement as whether a failed traffic light should flash red or go dark: there is no answer detached from context.

**So "is it safe" is a risk-and-policy judgement, not a technical fact.** For a high-stakes system, one whose agent can move money, send email, or execute code, "refuse rather than be hijacked" is often exactly the safety property you want, and aborting is the correct, safe behaviour. For a system whose mission is availability, a medical triage assistant or an emergency responder, an injection that can deny it service is a serious exploitable weakness, and the abort is the flaw. The same behaviour is a safety feature in one deployment and a vulnerability in another. The difference is not in the system; it is in the context.

**Which is the point of the section, for an assurance tool.** If "is it safe" is a risk judgement that belongs to the deployer, then the one thing an honest assurance tool must not do is make that judgement for them by flattening it into an overall yes or no. For a tool whose whole selling point is honest, bounded conclusions, collapsing the security and utility axes into a single "safe or not" scalar is itself the largest possible over-claim, and so the least safe thing it could do. The only honest conclusion it can offer is a bounded sentence: under attack condition Z, integrity holds, meaning no hijack, but availability fails, meaning the task is aborted, and whether that trade is acceptable depends on how much injection-triggered denial of service the deployment can tolerate. That is exactly what the comparison layer encodes when `joint_verdict` reads `utility_failed` alongside both the security and the utility arm reported honestly.

**It also settles how the report should present the thing.** A hard rule for the report layer follows directly. The report must not demote a `utility_failed` control to "not passed," because that would quietly make the deployer's "lost availability equals unsafe" value judgement for them, and that judgement is context-dependent, belongs to the user, and is not the tool's to make. Nor may the report bury the detail somewhere a reader reaches only by drilling down, because that lets the headline coverage read as a wall of green and makes the tool complicit in the over-claim that the system is safe. The one defensible course is to report the security-axis coverage as the fact it is, but to keep a joint-verdict caveat permanently beside it, so the number can never be read as "the system is safe," to lay the trade-off open, and to leave the decision to the context. The internal report view, the one aimed at operators and assessors, is built to do exactly this: the security-axis coverage stays factual, the joint-verdict caveat sits right next to it, and the two are read together, so nobody can wave the question of system safety away with a single number.
...
- `docs/adr/`: the complete arc of decisions, 0001 through 0017.
