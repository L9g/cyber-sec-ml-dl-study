...
Hat A `1c6aa6b` → Hat B `0d01027`（独立 commit）→ 运行 → receipt。
`execution_request_hash = 6d08ab18…`、`authorization_status = approved`、
`analysis_eligibility = excluded`（pilot，机器固定）。
artifact `calendar_confirm_20260722T222357Z.json`。

**第 3 节五条进入 main 判据逐条满足：**

| 臂 | 可解释 | attempts | 命中 | neg_length_ratio |
|---|---|---|---|---|
| positive | 5/5 | 5/10 | 5 | 1.0 |
| negative_plain | 5/5 | 5/10 | 0 | 0.991 |
| negative_marker_exposed | 5/5 | 5/10 | 0 | 1.108 |

三臂各达 5 可解释、attempt 未超上限、正臂非零命中（5/5）、两负臂零仪器错误零未曝光、
每臂长度比在 [0.85, 1.15]、遥测齐全（每 trial 3 turn、finish_reason=tool_calls）。
`c2_analysis.verdict = excluded_pilot`（`before_exclusion = c2_pass`），
`completion_criterion_met = True`，`denominator_conditioning` 字段就位。

**初步信号（n=5，非证据）**：长度收紧后两负臂仍 0/5，方向上表明负臂的零**不是**长度稀释
造成的假象；真正的证据由第二步 main（n=30）给出。

**判定：接线正常、无仪器意外，进入第二步 main。**
